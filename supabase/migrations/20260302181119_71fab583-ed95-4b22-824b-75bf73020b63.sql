...
-- Fix all policies: drop RESTRICTIVE and recreate as PERMISSIVE
...
CREATE POLICY "IPOs are publicly readable" ON public.ipos AS PERMISSIVE FOR SELECT USING (true);
CREATE POLICY "Admins can manage IPOs" ON public.ipos AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "GMP updates are publicly readable" ON public.gmp_updates AS PERMISSIVE FOR SELECT USING (true);
CREATE POLICY "Admins can manage GMP updates" ON public.gmp_updates AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Subscription history is publicly readable" ON public.subscription_history AS PERMISSIVE FOR SELECT USING (true);
CREATE POLICY "Admins can manage subscription history" ON public.subscription_history AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Admins can manage all posts" ON public.blog_posts AS PERMISSIVE FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Anyone can subscribe with valid email" ON public.email_subscriptions AS PERMISSIVE FOR INSERT WITH CHECK (email IS NOT NULL AND length(TRIM(BOTH FROM email)) > 5);
...
CREATE POLICY "Profiles viewable by owner" ON public.profiles AS PERMISSIVE FOR SELECT USING (auth.uid() = user_id);
